...
def innings_to_outs(ip):
    ...
    if "." in ip:
        whole_innings, partial_outs = ip.split(".")
        return int(whole_innings) * 3 + int(partial_outs)
    else:
        return int(ip) * 3
...
df["IP Outs"] = df["IP"].apply(innings_to_outs)
...
df["IP Rank"] = df["IP Outs"].rank(ascending=False)

#Create a weighted overall score
df["Overall Score"] = (
    (df["ERA Rank"] * 0.30) +
    (df["BAA Rank"] * 0.25) +
    (df["WHIP Rank"] * 0.30) +
